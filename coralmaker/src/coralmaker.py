...
class Corallite():
  ...
  def calculate_growth_vector(self):
    ...
    if self.number_of_living_neighbors > 2:
      ...
      for triangle in triangle_list:
        ...
        #print "v1, v2", v1, v2
        v_up = array( [ v1[0] * v2[1] - v1[1] * v2[0], v1[1]*v2[2] - v1[2]*v2[1], v1[2]*v2[0] - v1[0]*v2[2] ], float ) 
        ip = dot( v_up, self.upwardVector )
        #print "vup", v_up, "upward", self.upwardVector, "ip:", ip   
        if ( ip < 0 ) :
          v_up = array( [ v2[0] * v1[1] - v2[1] * v1[0], v2[1]*v1[2] - v2[2]*v1[1], v2[2]*v1[0] - v2[0]*v1[2] ], float ) 
    ...
    if self.colony.enhance_vertical_growth:
      ...
      cosine_theta = dot( self.upwardVector, verticalVector ) / linalg.norm( self.upwardVector ) * linalg.norm( verticalVector )
    ...
    if self.number_of_living_neighbors < self.colony.neighbor_count_threshold and self.min_dist > self.colony.min_distance_for_division:
      ...
      if False: 
        ...
        if len_move_vec > 0:
          ...
          move_vector += array( [ self.get_random_factor(),self.get_random_factor(),self.get_random_factor() ], float ) * self.move_ratio_per_tick  
  ...
  def branch(self):
    ...
    neighbor = self.connected_neighbor[0]
    child.pos = array( self.pos[:] + self.pos[:] - ( self.connected_neighbor[0].pos[:] + self.connected_neighbor[1].pos[:] ) / 2.0 )
    self.connect_with( child )
